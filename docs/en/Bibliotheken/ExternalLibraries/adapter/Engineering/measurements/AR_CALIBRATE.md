...
| `CO` | `Event` | | Atomic trigger for offset calibration (reads current raw value from `X`) |
| `CS` | `Event` | | Atomic trigger for scale calibration (reads current raw value from `X`) |
...
1. In state `REQ`, the function block (FB) is transitioned to state `CO` by receiving event `CO`. There, the offset is calculated and output via adapter `OFFSET`. The FB then immediately returns to state `REQ`.
2. Scale calibration is performed analogously via event `CS` and state `CS`.
...
- `REQ` → `CO`: when event `CO` is received
- `REQ` → `CS`: when event `CS` is received
- `CO` → `REQ`: always (condition `1`)
- `CS` → `REQ`: always (condition `1`)

## Technical Features

- **Atomic Event Triggering**: Calibration commands `CO` and `CS` are defined as direct event inputs, eliminating intermediate AX adapters and data-guard checks.
- **Bidirectional Calibration Adapters**: The adapters `OFFSET` and `SCALE` are bidirectional (`AR2`), allowing them to be written by the calibration FB and read by external modules.
...
- **Industrial Measurement Processing**: Calibration of pressure sensors, temperature sensors, or other analog transmitters where offset and gain need readjustment.
- **Measurement Chain Calibration**: After connecting a sensor, the entire chain can be linearized by establishing a zero point (offset) and a second reference point (scale).
- **Automated Recalibration**: Triggering event inputs `CO` and `CS` can compensate for sensor drift during operation.
...
The `AR_CALIBRATE` function block provides a robust solution for offset and scale calibration of analog signals. Thanks to its atomic event inputs and clear state machine, it integrates seamlessly into automation solutions and enables accurate, repeatable measurement corrections.
