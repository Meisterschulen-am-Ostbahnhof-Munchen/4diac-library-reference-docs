...
The block emits confirmation event `CNF` on every `REQ`. Event deduplication for unchanged values is handled at the adapter wrapper level (**ALI_SPLIT_SIGNED**) using `E_D_FF_ANY`.
...
- **ALI_SPLIT_SIGNED**: Composite FB wrapper featuring adapter interfaces (`adapter::types::unidirectional::ALI`) and automatic per-side event deduplication.
