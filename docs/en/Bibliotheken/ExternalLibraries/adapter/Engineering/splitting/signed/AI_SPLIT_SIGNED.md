...
The block encapsulates calculation FB **SPLIT_SIGNED_INT** and two **E_D_FF_ANY** deduplication blocks to emit events on plugs `NEG_MAG` and `POS_MAG` **only when the respective value actually changes**.
...
| Name | Type | Comment |
|---|---|---|
| `Y` | `adapter::types::unidirectional::AI` | Signed input value (adapter socket) |

### **Adapter Plugs (Output)**

| Name | Type | Comment |
|---|---|---|
| `NEG_MAG` | `adapter::types::unidirectional::AI` | Negative magnitude (`MAX(0, -Y)`), event on change only |
| `POS_MAG` | `adapter::types::unidirectional::AI` | Positive magnitude (`MAX(0, Y)`), event on change only |
...
- **AI_SPLIT_SIGNED**: Composite FB with adapter interfaces and automatic event deduplication.
