...
The block encapsulates calculation FB **SPLIT_SIGNED_LINT** and two **E_D_FF_ANY** D-Flip-Flop blocks to emit the initial event on the first `CLK` and thereafter emit events on plugs `NEG_MAG` and `POS_MAG` **only when the new input value differs from the stored output state** (`D <> Q`).
...
| Name | Type | Comment |
|---|---|---|
| `NEG_MAG` | `adapter::types::unidirectional::ALI` | Negative magnitude (`MAX(0, -Y)`), initial event on 1st call, thereafter on change only |
| `POS_MAG` | `adapter::types::unidirectional::ALI` | Positive magnitude (`MAX(0, Y)`), initial event on 1st call, thereafter on change only |
