...
The block encapsulates calculation FB **SPLIT_SIGNED_LINT** and two **E_D_FF_ANY** deduplication blocks to emit events on plugs `NEG_MAG` and `POS_MAG` **only when the respective value actually changes**.
...
- **ALI_SPLIT_SIGNED**: Composite FB with adapter interfaces and automatic event deduplication.
