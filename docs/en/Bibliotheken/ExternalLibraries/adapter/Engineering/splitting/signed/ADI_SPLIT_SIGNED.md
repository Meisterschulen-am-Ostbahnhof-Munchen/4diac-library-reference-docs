...
The block encapsulates calculation FB **SPLIT_SIGNED_DINT** and two **E_D_FF_ANY** deduplication blocks to emit events on plugs `NEG_MAG` and `POS_MAG` **only when the respective value actually changes**.
...
| Name | Type | Comment |
|---|---|---|
| `Y` | `adapter::types::unidirectional::ADI` | Signed input value (adapter socket) |
...
- **ADI_SPLIT_SIGNED**: Composite FB with adapter interfaces and automatic event deduplication.
