...
The block encapsulates calculation FB **SPLIT_SIGNED_SINT** and two **E_D_FF_ANY** D-Flip-Flop blocks to emit the initial event on the first `CLK` and thereafter emit events on plugs `NEG_MAG` and `POS_MAG` **only when the new input value differs from the stored output state** (`D <> Q`).
...
| Name | Type | Comment |
|---|---|---|
| `Y` | `adapter::types::unidirectional::AS` | Signed input value (AS adapter socket) |

### **Adapter Plugs (Output)**

| Name | Type | Comment |
|---|---|---|
| `NEG_MAG` | `adapter::types::unidirectional::AS` | Negative magnitude (`MAX(0, -Y)`), initial event on 1st call, thereafter on change only |
| `POS_MAG` | `adapter::types::unidirectional::AS` | Positive magnitude (`MAX(0, Y)`), initial event on 1st call, thereafter on change only |
...
- **AS_SPLIT_SIGNED**: Composite FB with adapter interfaces and D-Flip-Flop event filtering.
