...
The block encapsulates calculation FB **SPLIT_SIGNED_SINT** and two **E_D_FF_ANY** D-Flip-Flop blocks to emit events on plugs `NEG_MAG` and `POS_MAG` **only when the new input value differs from the stored output state** (`D <> Q`).
