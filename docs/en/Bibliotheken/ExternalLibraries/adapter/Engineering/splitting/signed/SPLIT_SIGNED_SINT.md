...
The block emits confirmation event `CNF` on every `REQ`. Event deduplication for unchanged values is handled at the adapter wrapper level (**AS_SPLIT_SIGNED**) using `E_D_FF_ANY`.
...
- **AS_SPLIT_SIGNED**: Composite FB wrapper featuring adapter interfaces (`adapter::types::unidirectional::AS`) and automatic per-side event deduplication.
