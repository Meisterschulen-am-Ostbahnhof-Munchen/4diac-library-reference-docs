...
The function block `AR_MAX` is used to determine the maximum value of two analog input signals transmitted via adapters (`AR`). It compares the data values of the two input sockets `IN0` and `IN1` and passes the larger value to the output plug `OUT`.
...
- **OUT** (Type: `adapter::types::unidirectional::AR`):
  The output adapter. It provides the maximum value $\max(\text{IN0.D1}, \text{IN1.D1})$ including the corresponding update event.
...
The computed maximum value is set on `OUT.D1` and emitted along with output event `OUT.E1`.
...
The `AR_MAX` is a useful selection block for calculating extreme values (maximum selection) in adapter-based IEC 61499 applications.
