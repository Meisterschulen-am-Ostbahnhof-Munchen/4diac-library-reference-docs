...
`ScrollFS_PHYS_Button` is structurally identical to [ScrollFS_PHYS_Softkey](ScrollFS_PHYS_Softkey.md) — the same
...
Identical to [ScrollFS_PHYS_Softkey](ScrollFS_PHYS_Softkey.md), with one difference: the 6 controls are
...
See [ScrollFS_PHYS_Softkey](ScrollFS_PHYS_Softkey.md), "Technical Details" section - everything described there
...
Same as [ScrollFS_PHYS_Softkey](ScrollFS_PHYS_Softkey.md) - no state of its own beyond what `Inner`
...
- **Versus `ScrollFS_PHYS_Softkey`**: identical except for the control type (`Button_IE`/
  `BT_PRESSED_LATCHED` instead of `Softkey_IE`/`SK_PRESSED`). See
  [ScrollFS_PHYS_Softkey](ScrollFS_PHYS_Softkey.md) for the full description.
...
`ScrollFS_PHYS_Button` offers the same finished scroll functionality as `ScrollFS_PHYS_Softkey`, just for
