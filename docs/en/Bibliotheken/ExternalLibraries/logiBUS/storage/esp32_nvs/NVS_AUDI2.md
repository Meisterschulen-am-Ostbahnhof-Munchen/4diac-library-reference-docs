...
![NVS_AUDI2](./NVS_AUDI2.svg)

* * * * * * * * * *

## Introduction

The **NVS_AUDI2** function block is used to load and store UDINT data in non-volatile storage (NVS) of an ESP32 via an adapter. It encapsulates initialization and access to a single NVS entry identified by a KEY.
...
| `DEFAULT_VALUE` | UDINT | Default value if key does not exist in NVS (Default: 0) |
...
| ------- | ---- | ------- |
| `VAL` | `adapter::types::bidirectional::AUDI2` (Socket) | Value (UDINT) |
