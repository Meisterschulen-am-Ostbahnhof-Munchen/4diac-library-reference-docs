...
The **NVS_AUI2** function block is used to load and store UINT data in non-volatile storage (NVS) of an ESP32 via an adapter. It encapsulates initialization and access to a single NVS entry identified by a KEY.
...
| `DEFAULT_VALUE` | UINT | Default value if key does not exist in NVS (Default: 0) |
...
| ------- | ---- | ------- |
| `VAL` | `adapter::types::bidirectional::AUI2` (Socket) | Value (UINT) |
