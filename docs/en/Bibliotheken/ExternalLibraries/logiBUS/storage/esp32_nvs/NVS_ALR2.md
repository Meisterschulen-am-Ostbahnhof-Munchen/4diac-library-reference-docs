...
![NVS_ALR2](./NVS_ALR2.svg)

* * * * * * * * * *

## Introduction

The **NVS_ALR2** function block is used to load and store LREAL data in non-volatile storage (NVS) of an ESP32 via an adapter. It encapsulates initialization and access to a single NVS entry identified by a KEY.
...
| `DEFAULT_VALUE` | LREAL | Default value if key does not exist in NVS (Default: 0.0) |
...
| ------- | ---- | ------- |
| `VAL` | `adapter::types::bidirectional::ALR2` (Socket) | Value (LREAL) |
