...
The **NVS_AS2** function block is used to load and store SINT data in non-volatile storage (NVS) of an ESP32 via an adapter. It encapsulates initialization and access to a single NVS entry identified by a KEY.
...
| `DEFAULT_VALUE` | SINT | Default value if key does not exist in NVS (Default: 0) |
...
| ------- | ---- | ------- |
| `VAL` | `adapter::types::bidirectional::AS2` (Socket) | Value (SINT) |
