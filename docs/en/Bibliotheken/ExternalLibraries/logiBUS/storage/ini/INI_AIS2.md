...
| DEFAULT_VALUE | STRING | Default value read if the key is not present in the file. Default: ''. |
...
| Name | Type | Description |
| ---- | ---- | ----------- |
| VAL | adapter::types::bidirectional::AIS2 | Bidirectional adapter for exchanging values. This adapter provides the read value as an output (DI1) and receives a write value as an input (DO1). |

## Functionality

The INI_AIS2 function block contains an internal INI function block (eclipse4diac::storage::INI) that performs the actual file operation. The network connections implement the following processes:
...
Thus, read and write access to the settings.ini file can be controlled via the AIS2 adapter.

## Technical Features

- The block uses a bidirectional AIS2 adapter that can both send and receive values.
- A STRING value is used as the default value.
