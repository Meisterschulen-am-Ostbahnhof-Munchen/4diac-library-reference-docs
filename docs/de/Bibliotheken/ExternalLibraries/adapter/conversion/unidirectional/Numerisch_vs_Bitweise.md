...
| Quelle ＼ Ziel            | → BOOL          | → ANY_BIT (außer BOOL)                                                     | → ANY_INT                                                                                             | → ANY_REAL                                                                |
| ------------------------ | ---------------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| **ANY_BIT** (außer BOOL) | LSB-Test          | Bitmuster-Übertragung (strukturell, kein Zahlenwert)                      | Bit-Reinterpretation — bei einem **breiteren** Ziel bleibt der Zahlenwert erhalten (Nullerweiterung); bei **gleicher Breite** bleibt zwar das Bitmuster erhalten, aber ein vorzeichenbehaftetes Ziel kann den Zahlenwert ändern (z. B. wird `0xFF` als `BYTE` zu `-1` als `SINT`, nicht zu `255`); ein **schmaleres** Ziel schneidet die oberen Bits ab | ⚠️ **Bit-Reinterpretation — KEIN Zahlenwert!** IEEE754-Fehlinterpretation |
| **BOOL**                 | Identität         | Numerisch `0`/`1`                                                         | numerisch (0/1)                                                                                       | numerisch (0.0/1.0) — Sonderfall, siehe unten                             |
| **ANY_INT**              | Boolesche Konvertierung (`!= 0`) | Bitmuster-Übertragung (erwartetes Verhalten für ein Bit-String-Ziel) | numerisch (Vorzeichenerweiterung/Nullerweiterung sicher, Verengung kann abschneiden)                  | **numerisch** (korrekter Cast)                                            |
| **ANY_REAL**             | Boolesche Konvertierung (`!= 0.0`) | Bit-Extraktion (beabsichtigt, z. B. Serialisierung via `F_REAL_TO_DWORD`) | numerisch (Rundung, `llrint`)                                                                         | numerisch (Auf-/Abrunden der Genauigkeit)                                 |

**Die auffälligste Falle** ist die Zelle **ANY_BIT (außer BOOL) → ANY_REAL** (rot markiert): `BYTE`/`WORD`/`DWORD`/`LWORD` als Quelle einer Umwandlung nach `REAL`/`LREAL`. In dieser Bibliothek betrifft das konkret zwei Bausteine:

Eine weitere, oben in der Matrix bereits vermerkte Falle betrifft `ANY_BIT` (außer BOOL) → `ANY_INT` bei gleicher Bitbreite: Das Bitmuster bleibt erhalten, aber ein vorzeichenbehaftetes Ziel kann den Zahlenwert ändern (z. B. wird `0xFF` als `BYTE` zu `-1` als `SINT`). Anders als bei `ANY_BIT`→`ANY_REAL` bleibt der resultierende Wert dabei aber im plausiblen Zahlenbereich und wird nicht durch eine IEEE754-Fehlinterpretation komplett sinnlos — der Fehler fällt beim Debuggen daher meist schneller auf.
