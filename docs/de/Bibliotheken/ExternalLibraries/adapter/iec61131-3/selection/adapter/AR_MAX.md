...
Der Funktionsbaustein `AR_MAX` dient der Ermittlung des Maximalwerts aus zwei analogen Eingangssignalen, die über Adapter (`AR`) übertragen werden. Er vergleicht die Datenwerte der beiden Eingangs-Sockets `IN0` und `IN1` und gibt den größeren der beiden Werte an den Ausgangs-Plug `OUT` weiter.
...
- **OUT** (Typ: `adapter::types::unidirectional::AR`):
  Der Ausgangsadapter. Er liefert den Maximalwert $\max(\text{IN0.D1}, \text{IN1.D1})$ inklusive des dazugehörigen Aktualisierungsereignisses.
...
$$\text{OUT.D1} = \max(\text{IN0.D1}, \text{IN1.D1})$$

Der ermittelte Maximalwert wird auf `OUT.D1` gelegt und zusammen mit dem Ausgangsereignis `OUT.E1` emittiert.
...
Der `AR_MAX` ist ein nützlicher Auswahlbaustein zur Bildung von Extremwerten (Höchstwertauswahl) in adapterbasierten IEC 61499 Anwendungen.
