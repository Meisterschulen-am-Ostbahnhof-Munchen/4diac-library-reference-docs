...
Der Baustein kapselt den Berechnungs-FB **SPLIT_SIGNED_DINT** sowie zwei D-Flip-Flop-Bausteine vom Typ **E_D_FF_ANY**, um das initiale Ereignis beim ersten `CLK` und danach Ereignisse auf den Plugs `NEG_MAG` und `POS_MAG` **nur bei tatsächlicher Wertänderung** der jeweiligen Seite auszulösen.
...
| Name | Typ | Kommentar |
|---|---|---|
| `Y` | `adapter::types::unidirectional::ADI` | Vorzeichenbehafteter Eingangswert (ADI-Adapter-Socket) |

### **Adapter-Plugs (Ausgang)**

| Name | Typ | Kommentar |
|---|---|---|
| `NEG_MAG` | `adapter::types::unidirectional::ADI` | Betrag der negativen Auslenkung (`MAX(0, -Y)`), initiales Ereignis beim 1. Aufruf, danach nur bei Wertänderung |
| `POS_MAG` | `adapter::types::unidirectional::ADI` | Betrag der positiven Auslenkung (`MAX(0, Y)`), initiales Ereignis beim 1. Aufruf, danach nur bei Wertänderung |
