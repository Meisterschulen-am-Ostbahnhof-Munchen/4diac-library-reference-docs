...
Der Baustein kapselt den Berechnungs-FB **SPLIT_SIGNED_DINT** sowie zwei Entprell-Bausteine vom Typ **E_D_FF_ANY**, um Ereignisse auf den Plugs `NEG_MAG` und `POS_MAG` **nur bei tatsächlicher Wertänderung** der jeweiligen Seite auszulösen.
...
| Name | Typ | Kommentar |
|---|---|---|
| `Y` | `adapter::types::unidirectional::ADI` | Vorzeichenbehafteter Eingangswert (AR/AX-Adapter) |

### **Adapter-Plugs (Ausgang)**

| Name | Typ | Kommentar |
|---|---|---|
| `NEG_MAG` | `adapter::types::unidirectional::ADI` | Betrag der negativen Auslenkung (`MAX(0, -Y)`), Event nur bei Änderung |
| `POS_MAG` | `adapter::types::unidirectional::ADI` | Betrag der positiven Auslenkung (`MAX(0, Y)`), Event nur bei Änderung |
...
- **ADI_SPLIT_SIGNED**: Composite FB mit Adapter-Schnittstellen und automatischer Event-Deduplizierung.
- **SPLIT_SIGNED_DINT**: Unterlagerter Basic FB für reine Signalberechnung ohne Adapter.

## Fazit

**ADI_SPLIT_SIGNED** bietet eine elegante, adapterbasierte und ereigniseffiziente Lösung zur Vorzeichen-Signalaufteilung in IEC 61499 Anwendungen.
