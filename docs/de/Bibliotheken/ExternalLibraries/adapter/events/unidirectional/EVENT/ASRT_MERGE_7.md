...
Der Funktionsblock **ASRT_MERGE_7** führt 7 unidirektionale ASRT-Adapter (Set/Reset/Toggle-Ereignisadapter, keine Nutzdaten) auf einen gemeinsamen Ausgangsadapter zusammen. Er ist die 7-Eingangs-Variante von [ASRT_MERGE_2](ASRT_MERGE_2.md) und wie dieser als generischer FB implementiert (`GenericClassName: 'GEN_ASRT_MERGE'`) — dieselbe C++-Basis (`CGenUnidirectMergeBase`), nur mit 7 statt 2 Sockets.
...
| Socket (Eingang 7) | `IN7` | `adapter::types::unidirectional::ASRT` | 7. Quelle. |
| Plug (Ausgang) | `OUT` | `adapter::types::unidirectional::ASRT` | Zusammengeführtes Signal. |

## Funktionsweise

Trifft `SET` bzw. `RESET` bzw. `TOGGLE` an einem der 7 Sockets (`IN1`, `IN2`, `IN3`, `IN4`, `IN5`, `IN6`, `IN7`) ein, wird es als gleichartiges Ereignis unverändert an `OUT` weitergereicht. Alle Ereignistypen werden unabhängig voneinander behandelt, und alle 7 Sockets werden gleichberechtigt behandelt — welcher Socket zuerst geprüft wird, folgt der Reihenfolge `IN1` … `IN7`, was nur bei mehreren innerhalb desselben Ausführungszyklus eintreffenden Ereignissen relevant wird. Da `ASRT` keinen Datenwert transportiert, gibt es nichts zu arbitrieren außer dem jeweiligen Ereignistyp selbst.
...
- **Beliebige Eingangsanzahl**: Dieselbe Implementierung deckt ASRT_MERGE_2 bis ASRT_MERGE_7 ab; für andere Eingangsanzahlen siehe `ASRT_MERGE_2`, `ASRT_MERGE_3`, `ASRT_MERGE_4`, `ASRT_MERGE_5`, `ASRT_MERGE_6`.
...
- **Zusammenführen mehrerer gleichwertiger Signalquellen** (z. B. 7 redundante oder alternative Auslösepfade) auf einen gemeinsamen ASRT-Adapterausgang.
...
- **`ASRT_MERGE_2`, `ASRT_MERGE_3`, `ASRT_MERGE_4`, `ASRT_MERGE_5`, `ASRT_MERGE_6`**: dieselbe generische Implementierung mit anderer Eingangsanzahl.
- [AE_MERGE_7](AE_MERGE_7.md), [ASR_MERGE_7](ASR_MERGE_7.md): dieselbe Zusammenführungslogik für die jeweils anderen unidirektionalen Ereignisadapter.

## Fazit

`ASRT_MERGE_7` liefert eine generisch implementierte Zusammenführung von 7 `ASRT`-Ereignisquellen auf einen gemeinsamen Adapterausgang und ergänzt die `GEN_ASRT_MERGE`-Familie um die 7-Eingangs-Variante.
