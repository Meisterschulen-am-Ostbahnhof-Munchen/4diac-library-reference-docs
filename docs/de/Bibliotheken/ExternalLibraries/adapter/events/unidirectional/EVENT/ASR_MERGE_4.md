...
![ASR_MERGE_4](ASR_MERGE_4.svg)

* * * * * * * * * *

## Einleitung

Der Funktionsblock **ASR_MERGE_4** führt 4 unidirektionale ASR-Adapter (Set/Reset-Ereignisadapter, keine Nutzdaten) auf einen gemeinsamen Ausgangsadapter zusammen. Er ist die 4-Eingangs-Variante von [ASR_MERGE_2](ASR_MERGE_2.md) und wie dieser als generischer FB implementiert (`GenericClassName: 'GEN_ASR_MERGE'`) — dieselbe C++-Basis (`CGenUnidirectMergeBase`), nur mit 4 statt 2 Sockets.
...
Trifft `SET` bzw. `RESET` an einem der 4 Sockets (`IN1`, `IN2`, `IN3`, `IN4`) ein, wird es als gleichartiges Ereignis unverändert an `OUT` weitergereicht. Alle Ereignistypen werden unabhängig voneinander behandelt, und alle 4 Sockets werden gleichberechtigt behandelt — welcher Socket zuerst geprüft wird, folgt der Reihenfolge `IN1` … `IN4`, was nur bei mehreren innerhalb desselben Ausführungszyklus eintreffenden Ereignissen relevant wird. Da `ASR` keinen Datenwert transportiert, gibt es nichts zu arbitrieren außer dem jeweiligen Ereignistyp selbst.
...
- **Beliebige Eingangsanzahl**: Dieselbe Implementierung deckt ASR_MERGE_2 bis ASR_MERGE_7 ab; für andere Eingangsanzahlen siehe `ASR_MERGE_2`, `ASR_MERGE_3`, `ASR_MERGE_5`, `ASR_MERGE_6`, `ASR_MERGE_7`.
...
- **Zusammenführen mehrerer gleichwertiger Signalquellen** (z. B. 4 redundante oder alternative Auslösepfade) auf einen gemeinsamen ASR-Adapterausgang.
...
- **`ASR_MERGE_2`, `ASR_MERGE_3`, `ASR_MERGE_5`, `ASR_MERGE_6`, `ASR_MERGE_7`**: dieselbe generische Implementierung mit anderer Eingangsanzahl.
- [AE_MERGE_4](AE_MERGE_4.md), [ASRT_MERGE_4](ASRT_MERGE_4.md): dieselbe Zusammenführungslogik für die jeweils anderen unidirektionalen Ereignisadapter.

## Fazit

`ASR_MERGE_4` liefert eine generisch implementierte Zusammenführung von 4 `ASR`-Ereignisquellen auf einen gemeinsamen Adapterausgang und ergänzt die `GEN_ASR_MERGE`-Familie um die 4-Eingangs-Variante.
