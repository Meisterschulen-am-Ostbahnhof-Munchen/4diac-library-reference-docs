...
Der Funktionsblock **ASR_MERGE_6** führt 6 unidirektionale ASR-Adapter (Set/Reset-Ereignisadapter, keine Nutzdaten) auf einen gemeinsamen Ausgangsadapter zusammen. Er ist die 6-Eingangs-Variante von [ASR_MERGE_2](ASR_MERGE_2.md) und wie dieser als generischer FB implementiert (`GenericClassName: 'GEN_ASR_MERGE'`) — dieselbe C++-Basis (`CGenUnidirectMergeBase`), nur mit 6 statt 2 Sockets.
...
Trifft `SET` bzw. `RESET` an einem der 6 Sockets (`IN1`, `IN2`, `IN3`, `IN4`, `IN5`, `IN6`) ein, wird es als gleichartiges Ereignis unverändert an `OUT` weitergereicht. Alle Ereignistypen werden unabhängig voneinander behandelt, und alle 6 Sockets werden gleichberechtigt behandelt — welcher Socket zuerst geprüft wird, folgt der Reihenfolge `IN1` … `IN6`, was nur bei mehreren innerhalb desselben Ausführungszyklus eintreffenden Ereignissen relevant wird. Da `ASR` keinen Datenwert transportiert, gibt es nichts zu arbitrieren außer dem jeweiligen Ereignistyp selbst.
...
- **Beliebige Eingangsanzahl**: Dieselbe Implementierung deckt ASR_MERGE_2 bis ASR_MERGE_7 ab; für andere Eingangsanzahlen siehe `ASR_MERGE_2`, `ASR_MERGE_3`, `ASR_MERGE_4`, `ASR_MERGE_5`, `ASR_MERGE_7`.
...
- **Zusammenführen mehrerer gleichwertiger Signalquellen** (z. B. 6 redundante oder alternative Auslösepfade) auf einen gemeinsamen ASR-Adapterausgang.
...
- **`ASR_MERGE_2`, `ASR_MERGE_3`, `ASR_MERGE_4`, `ASR_MERGE_5`, `ASR_MERGE_7`**: dieselbe generische Implementierung mit anderer Eingangsanzahl.
- [AE_MERGE_6](AE_MERGE_6.md), [ASRT_MERGE_6](ASRT_MERGE_6.md): dieselbe Zusammenführungslogik für die jeweils anderen unidirektionalen Ereignisadapter.

## Fazit

`ASR_MERGE_6` liefert eine generisch implementierte Zusammenführung von 6 `ASR`-Ereignisquellen auf einen gemeinsamen Adapterausgang und ergänzt die `GEN_ASR_MERGE`-Familie um die 6-Eingangs-Variante.
