...
`ScrollFS_PHYS_Button` ist strukturell identisch zu [ScrollFS_PHYS_Softkey](ScrollFS_PHYS_Softkey.md) — derselbe
...
Identisch zu [ScrollFS_PHYS_Softkey](ScrollFS_PHYS_Softkey.md), mit einem Unterschied: die 6 Bedienelemente sind
...
identisch zu `ScrollFS_PHYS_Softkey`.
...
Wie [ScrollFS_PHYS_Softkey](ScrollFS_PHYS_Softkey.md) — kein eigener Zustand außer dem in `Inner`
...
- **Gegenüber `ScrollFS_PHYS_Softkey`**: identisch bis auf den Bedienelement-Typ (`Button_IE`/
  `BT_PRESSED_LATCHED` statt `Softkey_IE`/`SK_PRESSED`). Siehe [ScrollFS_PHYS_Softkey](ScrollFS_PHYS_Softkey.md)
...
`ScrollFS_PHYS_Button` bietet dieselbe fertige Scroll-Funktionalität wie `ScrollFS_PHYS_Softkey`, nur für
